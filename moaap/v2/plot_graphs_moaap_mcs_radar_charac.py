...
def compute_mcs_metrics(mcs_dict):
    ...
    for mcs in all_mcs:
        ...
        if len(track) > 1:
            dists = (
                np.sqrt(np.diff(track[:, 0]) ** 2 + np.diff(track[:, 1]) ** 2)
                * 111.0
            )
...
mcs_eur_cpm_eval = open_mcs(
    "/CPMs/ICTP/EURR-3/evaluation/ERA5/output",
    start="2000-01",
    end="2009-12",
)
mcs_eur_cpm_hist = open_mcs(
    "/CPMs/ICTP/EURR-3/historical/ECEarth/output",
    start="2000-01",
    end="2009-12",
)
mcs_eur_rcm_eval = open_mcs(
    "/RCMs/ICTP/EUR-12/evaluation/ERA5/output",
    start="2000-01",
    end="2009-12",
)
mcs_eur_rcm_hist = open_mcs(
    "/RCMs/ICTP/EUR-12/historical/ECEarth/output",
    start="2000-01",
    end="2009-12",
)
...
cpm_eval_medians, cpm_eval_raw, n_cpm_eval = compute_mcs_metrics(
    mcs_eur_cpm_eval
)
cpm_hist_medians, cpm_hist_raw, n_cpm_hist = compute_mcs_metrics(
    mcs_eur_cpm_hist
)
rcm_eval_medians, rcm_eval_raw, n_rcm_eval = compute_mcs_metrics(
    mcs_eur_rcm_eval
)
rcm_hist_medians, rcm_hist_raw, n_rcm_hist = compute_mcs_metrics(
    mcs_eur_rcm_hist
)

# Unit formatting dictionary matching the reference image
units = {
    "Duration": "h",
    "DistanceTraveled": "km",
    "Severity": "E3 m^3",
...
# Normalize each variable by its maximum axis limit (1.5 * M_obs) so all axes have equal distance scale
...
# Plot GPM (Observations) with filling
ax.plot(
    angles,
    norm_gpm,
    color="#4daf4a",
...
# Plot CPM evaluation (Blue shades, no fill)
ax.plot(
    angles,
    norm_cpm_eval,
    color="#1f77b4",
...
# Plot CPM historical (Blue shades, no fill)
ax.plot(
    angles,
    norm_cpm_hist,
    color="#41b6c4",
...
# Plot RCM evaluation (Blue shades, no fill)
ax.plot(
    angles,
    norm_rcm_eval,
    color="#225ea8",
...
# Plot RCM historical (Blue shades, no fill)
ax.plot(
    angles,
    norm_rcm_hist,
    color="#081d58",
...
# Formatting Grid: set 20 concentric circular rings spaced equally
...
# Perimeter Labels placed at equal radial distance outside the outermost circle
...
# Save figure
path_out = (
    "/leonardo/home/userexternal/mdasilva/leonardo_work/MOAAP/paper/figs/v2"
)
