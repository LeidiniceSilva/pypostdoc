...
# Reading smn station
df = pd.read_csv(os.path.join('{0}/SMN/hourly/'.format(path), 'PP_SMN_MIN_2018-01-01_2021-12-31.csv'))
df['Date'] = pd.to_datetime(df['Date'])
df = df.set_index('Date').resample('D').sum()
df.iloc[:,:].to_csv('{0}/SMN/hourly/PP_SMN_D_2018-01-01_2021-12-31.csv'.format(path))
...
for idx in range(0, 72):
	...
	# Reading smn station
	data = np.loadtxt(os.path.join('{0}/SMN/hourly/'.format(path), 'PP_SMN_H_2018-01-01_2021-12-31.csv'), dtype='str', delimiter=',', unpack=True)
	data = data[:,1:]
	...
	nc_output = '{0}/SMN/hourly/nc/pre_{1}_H_2018-01-01_2021-12-31.nc'.format(path, smn_i[idx+1][0])
