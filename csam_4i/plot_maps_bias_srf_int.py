...
def import_obs(param, domain, dataset):
	...
	arq   = '{0}/user/mdasilva/SAM-3km/post_evaluate/obs/{1}_int_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
...
def import_sam_3km(param, domain, dataset):
	...
	arq   = '{0}/user/mdasilva/SAM-3km/post_evaluate/rcm/{1}_int_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
...
def import_csam_4i(param, domain, dataset):
	...
	arq   = '{0}/user/mdasilva/CSAM-4i/post_evaluate/{1}_int_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
