...
if len(sys.argv) > 1:
    ...
    RCMf = nc(os.path.join('{0}/ctrl/output'.format(path),'psl_Otis_ctrl_2023101900.nc'), mode='r')
    
lat  = RCMf.variables['xlat'][:,:]
lon  = RCMf.variables['xlon'][:,:]
rcm = RCMf.variables['psl'][:,:]
...
for i in range(0, rcm.shape[0]):
	...
	ax.set_title(u'MSLP {0} UTC'.format(iso8601_t[i]))
	configure_subplot(ax)

	# Path out to save figure
	path_out = '{0}/figs'.format(path)
	name_out = 'pyplt_maps_psl_RegCM5_{0}.png'.format(iso8601_s[i])
