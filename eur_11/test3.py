...
def import_obs(param, dataset):
	...
	arq   = '{0}/user/mdasilva/EUR-11/postproc/obs/{1}_freq_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
...
def import_rcm(param, dataset):
	...
	arq   = '{0}/user/mdasilva/EUR-11/postproc/rcm/{1}_freq_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
...
plt_map = map.contourf(xx, yy, mbe_noto_cpc_jan[0][0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(a) NoTo - CPC Jan', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 2, 2)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wsm5_cpc_jan[0][0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(b) WSM5 - CPC Jan', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 2, 3)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wsm7_cpc_jan[0][0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(c) WSM7 - CPC Jan', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 2, 4)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wdm7_cpc_jan[0][0], levels=levs, cmap=cm.BrBG, extend='neither') 
