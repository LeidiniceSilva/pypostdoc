...
def import_obs(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/EUR-11/post_evaluate/obs/{1}_freq_{2}_{3}_{4}_{5}_lonlat.nc'.format(path, param, domain, dataset, season, dt)	
...
def import_rcm(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/EUR-11/post_evaluate/rcm/{3}/{1}_freq_{2}_{3}_RegCM5_{4}_{5}_lonlat.nc'.format(path, param, domain, dataset, season, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
...
if freq == 'hourly':
	...
	legend = 'Frequency of hourly precipitation (%)'
else:
	levs = np.arange(-50, 60, 10)
	legend = 'Frequency of daily precipitation (%)'
