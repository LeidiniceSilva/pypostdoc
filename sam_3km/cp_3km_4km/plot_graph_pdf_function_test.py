...
def import_obs(param, domain, dataset):
	...
	for station in range(1, 567):
		...
		arq    = xr.open_dataset('{0}/user/mdasilva/SAM-3km/post_evaluate/obs/'.format(path) + '{0}_{1}_{2}_{3}_lonlat.nc'.format(param, domain, dataset, dt))
		data   = arq[param]
...
def import_sam_3km(param, domain, dataset):
	...
	for station in range(1, 567):
		...
		arq    = xr.open_dataset('{0}/user/mdasilva/SAM-3km/post_evaluate/rcm/'.format(path) + '{0}_{1}_{2}_{3}_lonlat.nc'.format(param, domain, dataset, dt))
		data   = arq[param]
		latlon = data.sel(lat=slice(inmet[station][2]-0.03,inmet[station][2]+0.03),lon=slice(inmet[station][3]-0.03,inmet[station][3]+0.03)).mean(('lat','lon'))
		time   = latlon.sel(time=slice('2018-06-01','2021-05-31'))
		var    = time.values
...
plt.yscale('log')
plt.xlabel('Frequency (#)', fontsize=font_size, fontweight='bold')
