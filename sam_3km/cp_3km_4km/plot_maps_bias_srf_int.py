...
def import_csam_4i(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/CSAM-4i/post_evaluate/{1}_int_{2}_{3}_{4}_{5}_lonlat.nc'.format(path, param, domain, dataset, season, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
...
plt_map = map.contourf(xx, yy, mbe_reg_ictp_3km_cpc[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both') 
plt.title(u'(a) RegCM5-3km - CPC', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 2)  
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_reg_ictp_4km_cpc[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both') 
plt.title(u'(b) RegCM5-4km - CPC', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 3)  
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_reg_usp_cpc[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both') 
plt.title(u'(c) RegCM4 - CPC', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 4)  
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wrf_ncar_cpc[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both') 
plt.title(u'(d) WRF-NCAR - CPC', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 5)  
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wrf_ucan_cpc[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both') 
plt.title(u'(e) WRF-UCAN - CPC', loc='left', fontsize=font_size, fontweight='bold')

x = fig.add_subplot(2, 5, 6)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_reg_ictp_3km_era5[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both')
plt.title(u'(f) RegCM5-3km - ERA5', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 7)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_reg_ictp_4km_era5[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both')
plt.title(u'(g) RegCM5-4km - ERA5', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 8)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_reg_usp_era5[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both')
plt.title(u'(h) RegCM4 - ERA5', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 9)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wrf_ncar_era5[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both')
plt.title(u'(i) WRF-NCAR - ERA5', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(2, 5, 10)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_wrf_ucan_era5[0][0], levels=np.arange(-24, 28, 4), cmap=cm.BrBG, extend='both')
