...
def open_dat_file_v1(dataset, yr_init, yr_end):
	...
	for yr in range(yr_init, yr_end+1):
		...
		data = read_dat_file('{0}/post_cyclone/ECv2/{1}/track/resultado_{2}.dat'.format(path, dataset, yr))
...
ax1.set_xlabel('Pressure center (hPa)', fontsize=font_size, fontweight='bold')
ax1.set_ylabel('Frequency', fontsize=font_size, fontweight='bold')
ax1.set_ylim(0, 60)
ax1.axvspan(1012, 1018, color='k', alpha=0.1)
...
ax2.set_ylim(0, 60)
ax2.axvspan(1014, 1022, color='k', alpha=0.1)
