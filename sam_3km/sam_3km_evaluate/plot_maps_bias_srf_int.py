...
def import_obs(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/SAM-3km_v5/post/obs/{1}_int_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
...
def import_rcm(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/SAM-3km_v5/post/rcm/{1}_int_{2}_{3}_{4}_lonlat.nc'.format(path, param, domain, dataset, dt)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
