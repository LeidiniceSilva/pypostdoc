...
def import_obs(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/SAM-3km_v5/post/obs/p99_{1}_{2}_{3}_lonlat.nc'.format(path, domain, dataset, season)	
...
def import_rcm(param, domain, dataset, season):
	...
	arq   = '{0}/user/mdasilva/SAM-3km_v5/post/rcm/p99_{1}_{2}_{3}_lonlat.nc'.format(path, domain, dataset, season)	
	data  = netCDF4.Dataset(arq)
	var   = data.variables[param][:] 
	lat   = data.variables['lat'][:]
	lon   = data.variables['lon'][:]
...
plt_map = map.contourf(xx, yy, mbe_jan[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(a) RegCM5-CPC Jan', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 2)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_feb[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(b) RegCM5-CPC Feb', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 3)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_mar[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(c) RegCM5-CPC Mar', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 4)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_apr[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(d) RegCM5-CPC Apr', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 5)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_may[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(e) RegCM5-CPC May', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 6)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_jun[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(f) RegCM5-CPC Jun', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 7)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_jul[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(g) RegCM5-CPC Jul', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 8)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_aug[0], levels=levs, cmap=cm.BrBG, extend='neither') 
plt.title(u'(h) RegCM5-CPC Aug', loc='left', fontsize=font_size, fontweight='bold')

ax = fig.add_subplot(3, 3, 9)
map, xx, yy = basemap(lat, lon)
plt_map = map.contourf(xx, yy, mbe_sep[0], levels=levs, cmap=cm.BrBG, extend='neither') 
