...
df = pd.read_csv('{0}/post_cyclone/ECyclone/ECyclone_{1}/tracking_density_{1}_{2}.txt'.format(path, dataset, legend), sep=" ")
...
ax.set_title('{0} {1}'.format(dataset, legend), loc='left', fontsize=font_size, fontweight='bold')

cf = ax.contourf(scipy.ndimage.zoom(dens.lon,1.5),scipy.ndimage.zoom(dens.lat,1.5),scipy.ndimage.zoom(dens,1.5).T/4*1e6,colorb,transform=ccrs.PlateCarree(),extend='max',cmap='rainbow')
...
name_out = 'pyplt_genesis_density_EC_{0}_{1}_SAM-3km_2018-2021.png'.format(dataset, legend)
