...
var_data, lat, lon = load_data("/leonardo/home/userexternal/mdasilva/leonardo_work/CORDEX5/pycordexer/ICTP/ERA5/evaluation/r1i1p1f1/RegCM5-0/v1-r1/fx/{0}/{0}_CSAM-3_ERA5_evaluation_r1i1p1f1_ICTP_RegCM5-0_v1-r1_fx.nc".format(var))
...
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(11, 6), subplot_kw={'projection': ccrs.PlateCarree()})
font_size = 10
...
ax.set_title("(a) RegCM5 CTRL {0}".format(var), loc='left', fontsize=font_size, fontweight='bold')
im = ax.contourf(lon, lat, var_data, np.arange(0, 105, 5), cmap="gist_ncar_r", transform=ccrs.PlateCarree())
cbar = plt.colorbar(im, ax=ax, orientation='horizontal', shrink=0.7, pad=0.1)
cbar.set_label('Urban area fraction (%)', fontweight='bold')
...
ax.grid(c='gray', ls='--', alpha=0.5)  
ax.text(-42, -36, u'\u25B2 \nN', color='black', fontsize=font_size, fontweight='bold')
ax.set_title("(b) RegCM5 URBAN {0}".format(var), loc='left', fontsize=font_size, fontweight='bold')
im = ax.contourf(lon, lat, var_data_urb, np.arange(0, 105, 5), cmap="gist_ncar_r", transform=ccrs.PlateCarree())
cbar = plt.colorbar(im, ax=ax, orientation='horizontal', shrink=0.7, pad=0.1)
cbar.set_label('Urban area fraction (%)', fontweight='bold')

# Path out to save figure
path_out = '/leonardo/home/userexternal/mdasilva/leonardo_work/CORDEX5/figs/urban/paper'
name_out = 'pyplt_maps_{0}_RegCM5_CSAM-3_2000-2009.png'.format(var)
